...
def explain_decision(rule_id, decision_data):
    """
    Generate an explanation for a decision based on the rule that was triggered.
    
    Args:
        rule_id (str): The ID of the rule that was triggered
    ...
    if rule_id not in rules:
        return f"Rule with ID '{rule_id}' not found in the Decision Center."
    ...
    if "ActionRule" in rule_id:
        explanation = explain_action_rule(rule_def, decision_data)
    elif "DecisionTable" in rule_id:
        explanation = explain_decision_table(rule_def, decision_data)
    else:
        explanation = f"Unknown rule type for rule ID: {rule_id}"
...
def main():
    ...
    # Example rule ID that was triggered for this decision
    triggered_rule_id = "brm.ActionRule:45:45"
    
    # Generate explanation
    explanation = explain_decision(triggered_rule_id, decision_data)
